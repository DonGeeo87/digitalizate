    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50">
      ...
      <Dialog open={showOnboarding} onOpenChange={setShowOnboarding}>
        <DialogContent className="sm:max-w-md border-0 shadow-2xl">
          <DialogHeader>
            <div className="mx-auto w-16 h-16 bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl flex items-center justify-center text-3xl mb-4 shadow-lg">
              🚀
            </div>
            <DialogTitle className="text-2xl text-center text-slate-900">¡Bienvenido a Digitalízate!</DialogTitle>
      ...
      <nav className="sticky top-0 z-50 bg-white/70 backdrop-blur-md border-b border-slate-200/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg text-slate-900">Digitalízate</span>
          </div>

          {/* Desktop nav */}
          <div className="hidden md:flex items-center gap-6">
            <a href="#como-funciona" className="text-sm text-slate-500 hover:text-slate-900 transition-colors">Cómo funciona</a>
            <a href="#categorias" className="text-sm text-slate-500 hover:text-slate-900 transition-colors">Categorías</a>
            {profile ? (
              <Link href={`/desafio/${firstChallenge.slug}`}>
                <Button size="sm" className="bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-md shadow-green-500/20">
                  {firstChallenge.iconEmoji} Continuar desafío
                </Button>
              </Link>
            ) : (
              <Button size="sm" className="bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-md shadow-green-500/20" onClick={() => setShowOnboarding(true)}>
                Empezar gratis
              </Button>
            )}
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-slate-200/50 bg-white px-4 py-4 space-y-3">
            <a href="#como-funciona" className="block text-sm text-slate-500 py-2" onClick={() => setMenuOpen(false)}>Cómo funciona</a>
            <a href="#categorias" className="block text-sm text-slate-500 py-2" onClick={() => setMenuOpen(false)}>Categorías</a>
            {profile ? (
              <Link href={`/desafio/${firstChallenge.slug}`} onClick={() => setMenuOpen(false)}>
                <Button className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white">Continuar desafío</Button>
              </Link>
            ) : (
              <Button className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white" onClick={() => { setMenuOpen(false); setShowOnboarding(true); }}>Empezar gratis</Button>
            )}
          </div>
        )}
      </nav>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-green-600 via-emerald-600 to-green-700 text-white px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto text-center space-y-8">
            ...
            {profile && (
              <Card className="max-w-sm mx-auto bg-white/80 backdrop-blur border-green-500/10 shadow-lg shadow-green-500/5">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
              {profile ? (
                <>
                  <Link href={`/desafio/${firstChallenge.slug}`}>
                    <Button size="lg" className="text-base px-8 py-6 bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg shadow-green-500/25 hover:shadow-xl transition-all">
                      {firstChallenge.iconEmoji} {firstChallenge.title}
                      <ArrowRight className="w-5 h-5 ml-2" />
                    </Button>
                  </Link>
                  <Link href="/dashboard">
                    <Button size="lg" variant="outline" className="text-base px-8 py-6 border-2 border-slate-200 text-slate-700 hover:bg-green-500/5 hover:border-green-500/30 transition-all">
                      📊 Mi progreso
                    </Button>
                  </Link>
                </>
                ...
                <>
                  <Button size="lg" className="text-base px-10 py-6 bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg shadow-green-500/25 hover:shadow-xl transition-all" onClick={() => setShowOnboarding(true)}
      ...
      {/* ─── CÓMO FUNCIONA ─── */}
      <section id="como-funciona" className="py-16 sm:py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="inline-flex items-center gap-2 px-4 py-2 bg-green-500/10 rounded-full text-green-600 text-sm font-medium mb-4">
              <Target className="w-4 h-4" />
              Cómo funciona
            </span>
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-900">Así de simple</h2>
            <p className="text-slate-500 mt-3 max-w-md mx-auto">Olvídate de cursos de 40 horas. Esto es acción pura.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-6 sm:gap-8">
            {[
              { step: "01", icon: "📱", title: "Eliges un desafío", desc: "Ves el desafío del día y dices 'lo hago'. Son 5 minutos.", gradient: "from-green-500 to-emerald-600" },
              { step: "02", icon: "⚡", title: "Haces la acción", desc: "Pasos guiados. Abres la app, configuras, publicas. Sin rodeos.", gradient: "from-amber-500 to-orange-500" },
              { step: "03", icon: "🏆", title: "Ganas tu badge", desc: "Completaste. Te llevas un badge que puedes compartir en tus redes.", gradient: "from-blue-500 to-purple-500" },
            ].map((item) => (
              <div key={item.step} className="group relative">
                <div className="absolute -inset-0.5 bg-gradient-to-br from-green-500/10 to-green-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity blur-sm" />
                <Card className="relative border-0 shadow-sm bg-white overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
      ...
      <section id="categorias" className="py-16 sm:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="inline-flex items-center gap-2 px-4 py-2 bg-purple-500/10 rounded-full text-purple-600 text-sm font-medium mb-4">
              <Trophy className="w-4 h-4" />
              60 desafíos
            </span>
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-900">¿Qué puedes lograr?</h2>
            <p className="text-slate-500 mt-3 max-w-md mx-auto">Tú eliges por dónde empezar según lo que más le duela a tu negocio hoy.</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
            {[
              ...
              <div key={cat.title} className="group transition-all duration-300 hover:-translate-y-1">
                <Card className="relative border-0 overflow-hidden bg-white shadow-sm hover:shadow-xl transition-all">
                  ...
                  <CardContent className="p-5 sm:p-6 flex gap-4 items-start">
                    ...
                    <div>
                      ...
                      <span className="inline-block text-xs font-semibold text-green-600 mt-3 bg-green-500/10 px-3 py-1 rounded-full">
                        {cat.count}
                      </span>
                    </div>
                  </CardContent>
      ...
      <section className="relative py-16 sm:py-24 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-green-500/5 via-white to-blue-500/5" />
        ...
        <div className="relative max-w-lg mx-auto text-center px-4 space-y-8">
          ...
          <div className="bg-white/80 backdrop-blur rounded-2xl p-6 sm:p-8 shadow-lg border border-slate-200/50 space-y-4">
          ...
          <Button size="lg" className="text-base px-10 py-6 bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg shadow-green-500/25 hover:shadow-xl transition-all"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid sm:grid-cols-3 gap-8">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 bg-gradient-to-br from-green-500 to-emerald-600 rounded-lg flex items-center justify-center text-white font-bold text-xs">D</div>
                <span className="font-bold text-slate-900">Digitalízate</span>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-2 text-sm text-slate-500">
                ...
                <a href="#categorias" className="hover:text-green-600 transition-colors">Desafíos</a>
                <button onClick={() => setShowOnboarding(true)} className="text-left hover:text-green-600 transition-colors">Empezar</button>
              </div>
            </div>
            <div className="space-y-3">
              <h4 className="font-semibold text-slate-900">Código Guerrero Dev</h4>
              <p className="text-sm text-slate-500">Hecho en Chile 🇨🇱 para Latam 🌎</p>
              <div className="flex gap-2 text-lg mt-2">
                <span>🇨🇱</span><span>🇲🇽</span><span>🇨🇴</span><span>🇦🇷</span><span>🇵🇪</span>
              </div>
            </div>
          </div>
          <div className="border-t border-slate-200/50 mt-8 pt-8 text-center text-sm text-slate-500">
